...
public class InventoryActions : MonoBehaviour {
	...
	private string calculateLocalTS(string serverTS) {
		...
		DateTime serverPurchaseTimeUTC = serverPurchaseTime.AddHours (calcOffsetHours());
	...
	/*
	 * should produce correct timestmaps 99.9% of tht time...
	 * but will result in Timestamps being off by 1 hour on the transition days to/from daylight savings
	 */
	private int calcOffsetHours() {

		bool firstSundayPassed = false;

		DateTime DaylightSavingStartTime = new DateTime(DateTime.Now.Year,3,1);
		DateTime DaylightSavingEndTime = new DateTime(DateTime.Now.Year,11,1);

		for (int i=0; i<14; i++) {
			if (!firstSundayPassed) {
				if (DaylightSavingStartTime.DayOfWeek == DayOfWeek.Sunday) {
			...
			DaylightSavingStartTime = DaylightSavingStartTime.AddDays(1);
		}

		for (int i=0; i<7; i++) {							
			if (DaylightSavingEndTime.DayOfWeek == DayOfWeek.Sunday) {
				break;
			}
			DaylightSavingEndTime = DaylightSavingEndTime.AddDays(1);

		}

//		Debug.Log ("Daylight Saving Start Date This Year: " + DaylightSavingStartTime.ToShortDateString ());
//		Debug.Log ("Daylight Saving End Date This Year: " + DaylightSavingEndTime.ToShortDateString ());
			
		if (DateTime.Now > DaylightSavingStartTime && DateTime.Now < DaylightSavingEndTime) {
			return 4;
		} else {
			return 5;
		}
